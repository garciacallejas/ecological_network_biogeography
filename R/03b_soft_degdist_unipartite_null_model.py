...
for mat_i in range(0, len(list_unipartite_matrices_files)):
    ...
    graph.ensemble_sampler(number_random_samples, cpu_n=5, output_dir="../null_adjacency_matrices/soft_degdist_null_unipartite_adjacency_matrices")
    
    # Turning the edge lists into an adjacency matrices with node names 
    for sample_i in range(0, number_random_samples):
    
        # Load edge_list
        edgelist_ens = np.loadtxt("../null_adjacency_matrices/soft_degdist_null_unipartite_adjacency_matrices/"+repr(sample_i)+".txt")
        ...
        np.savetxt('../null_adjacency_matrices/soft_degdist_null_unipartite_adjacency_matrices/' + network_name + "_sample_" + repr(sample_i) + '.csv',new_unip_mat_names , fmt='%s', delimiter=',')
        
        # Remove the edge list
        os.remove("../null_adjacency_matrices/soft_degdist_null_unipartite_adjacency_matrices/"+repr(sample_i)+".txt")
