...
# network metrics for soft-constrained degdist adjacency matrices

# NOTE: you will see that these metrics are stored directly in a single
# .csv file for unipartite networks and one file for bipartite ones.
# This is unlike the connectance null model and the hard-degdist null model,
# that store every network independently. This is for legacy reasons:
# I started with this null model and the results were created this way, 
# and only later implemented the other two null models. It does not
# interfere with result processing, as the other metrics are processed
# and joined together just like this one.
...
unipartite.null.path <- "data/null_adjacency_matrices/soft_degdist_null_unipartite_adjacency_matrices/"
bipartite.null.path <- "data/null_adjacency_matrices/soft_degdist_null_bipartite_adjacency_matrices/"
...
if(calc.unipartite){
  ...
  my.null.metrics <- foreach(i.null = 1:length(uni.null.matrices.list), 
                                  ...
                                  )) %dopar% 
    {
      ...
      my.null.net.name.full <- substr(uni.null.matrices.list[i.null],start = 1,
                                      stop = (nchar(uni.null.matrices.list[i.null])-4))
  ...
  write.csv2(my.null.long,paste("results/soft_degdist_null_unipartite_metrics/unipartite_network_metrics_soft_degdist_null.csv",sep=""),row.names = F)
...
if(calc.bipartite){
  ...
  my.null.long <- bind_rows(my.null.metrics) %>%
    pivot_longer(richness:link.density,names_to = "metric",values_to = "value")
  write.csv2(my.null.long,paste("results/soft_degdist_null_bipartite_metrics/bipartite_network_metrics_soft_degdist_null.csv",sep=""),row.names = F)
